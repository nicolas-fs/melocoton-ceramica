...
export default function CartSidebar() {
  const {
    items, estaAbierto, cerrarCarrito,
        ...
        <div className="flex items-center justify-between px-5 py-4 border-b border-melocoton-200">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-xl text-tierra-900">
              Tu carrito{' '}
              {cantidadTotal > 0 && (
                <span className="font-sans text-sm text-melocoton-500">({cantidadTotal})</span>
              )}
            </h2>
          </div>
          <button
            onClick={cerrarCarrito}
            className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-melocoton-200 transition-colors"
          >
        ...
        <div className="flex-1 overflow-y-auto scrollbar-thin px-5 py-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center gap-4">
              ...
              <Link href="/catalogo" onClick={cerrarCarrito} className="btn-primary mt-2">
                Ver catálogo
              </Link>
            </div>
          ) : (
            <ul className="space-y-4">
              {items.map(item => (
                <li
                  key={item.productoId}
                  className="flex gap-3 bg-white rounded-xl p-3 shadow-sm border border-melocoton-100"
                >
                  <div className="w-16 h-16 rounded-lg overflow-hidden flex-shrink-0">
                    <Image
                      src={item.imagen || 'https://placehold.co/64x64/faecd8/b07040?text='}
                      alt={item.titulo}
                      width={64}
                      height={64}
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-sans text-sm font-medium text-tierra-900 truncate leading-snug">
                      {item.titulo}
                    </p>
                    <p className="font-serif text-base text-melocoton-600 mt-0.5">
                      {formatearPrecio(item.precio)}
                    </p>
                    <div className="flex items-center gap-2 mt-2">
                      <button
                        onClick={() => actualizarCantidad(item.productoId, item.cantidad - 1)}
                        className="w-6 h-6 rounded-full border border-melocoton-300 flex items-center justify-center hover:bg-melocoton-100 transition-colors"
                      >
                        <Minus className="w-3 h-3 text-tierra-600" />
                      </button>
                      <span className="font-sans text-sm w-5 text-center text-tierra-900">
                        {item.cantidad}
                      </span>
                      <button
                        onClick={() => actualizarCantidad(item.productoId, item.cantidad + 1)}
                      ...
                      <span className="ml-auto font-sans text-xs text-tierra-400">
