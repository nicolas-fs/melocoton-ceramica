...
// Limite de productos por carga — reducido para mejorar velocidad
// Cuando se filtra por categoria o busqueda no aplica limite (son pocos resultados)
const LIMITE_SIN_FILTRO = 24;
...
export default async function CatalogoPage({ searchParams }: Props) {
  ...
  // Solo aplicar limite cuando no hay filtros activos
  const tieneFiltro = catValida || busqueda || promo === 'true';
  const limite = tieneFiltro ? undefined : LIMITE_SIN_FILTRO;

  let productos = await obtenerProductos({ categoria: catValida, busqueda, limite });

  // Filtrar solo productos en promocion
  if (promo === 'true') {
      ...
      <div className="container-mel py-12">
        <div className="flex flex-col lg:flex-row gap-8">
          ...
          <main className="flex-1">
            <div className="flex items-center justify-between mb-8 flex-wrap gap-3">
              <h2 className="font-serif text-2xl text-tierra-900">
                ...
                <span className="ml-3 font-sans text-base text-tierra-400 font-normal">
                  ({productos.length} {productos.length === 1 ? 'pieza' : 'piezas'}{!tieneFiltro ? ' — mostrando primeras 24' : ''})
                </span>
              </h2>
              {promo === 'true' && (
            ...
            {/* Boton ver todos cuando hay limite activo */}
            {!tieneFiltro && productos.length >= LIMITE_SIN_FILTRO && (
              <div className="text-center mt-12">
                <p className="font-sans text-sm text-tierra-500 mb-4">
                  Mostrando las primeras {LIMITE_SIN_FILTRO} piezas. Usá los filtros para encontrar lo que buscás.
                </p>
                <a
                  href="/catalogo?orden=destacados"
