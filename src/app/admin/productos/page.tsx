...
// Reduce fotos de 5-10MB a ~200-400KB sin pérdida visible.
// Usa canvas para reescalar y re-encodear como JPEG.
function comprimirImagen(
  file: File,
  maxPx = 1200,   // lado máximo en píxeles
...
// ── Modal crear/editar con upload de imagen ───────────────
function ProductoModal({ producto, onGuardar, onCerrar, guardando }: {
  ...
  function handleFileUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    // Comprimir la imagen antes de guardar (reduce tamaño ~80% sin pérdida visible)
  ...
  async function submit(e: React.FormEvent) {
    ...
    await onGuardar({
      ...
      imagenes:  f.imagenes ? [f.imagenes] : [],
        ...
        <form onSubmit={submit} className="p-6 space-y-4">
          ...
          <div>
            ...
            <input value={f.titulo} onChange={e => set('titulo', e.target.value)}
                   className="form-input" placeholder='Taza "Lo estás haciendo bien"' required />
            {f.titulo && (
          ...
          {/* CAMBIO 1.2: Upload de imagen desde archivo */}
          <div>
            <label className="form-label">Foto del producto</label>
            <div className="flex gap-3 items-start">

              {/* Área de drag/click para subir */}
              <label
                htmlFor="img-upload"
                className="flex-1 flex flex-col items-center justify-center h-28 border-2 border-dashed border-melocoton-300 rounded-xl cursor-pointer hover:border-melocoton-500 hover:bg-melocoton-50 transition-all"
              >
                <Upload className="w-6 h-6 text-melocoton-400 mb-1" />
                <p className="text-sm text-tierra-700 font-medium">Elegir foto</p>
                <p className="text-xs text-tierra-400 text-center px-3">JPG, PNG o WebP — máx. 10 MB</p>
                <input
                  id="img-upload"
                  ref={fileInputRef}
                  type="file"
              ...
              {preview && (
                <div className="relative flex-shrink-0">
                  ...
                  <img
                    src={preview}
                    alt="Vista previa"
                    className="w-24 h-24 rounded-xl object-cover border-2 border-melocoton-200"
                    onError={() => setPreview(null)}
                  />
                  <button
                    type="button"
                    onClick={clearImage}
                    className="absolute -top-2 -right-2 w-5 h-5 bg-red-500 text-white rounded-full flex items-center justify-center text-[10px] hover:bg-red-600 font-bold leading-none"
                    title="Quitar imagen"
                  >
                    x
                  </button>
            ...
            {/* Error de validación */}
            {uploadError && (
              <p className="text-xs text-red-500 mt-1.5 bg-red-50 border border-red-200 rounded-lg px-3 py-1.5">
                {uploadError}
              </p>
            )}
            ...
            <details className="mt-2">
              <summary className="text-xs text-tierra-400 cursor-pointer hover:text-tierra-600 select-none">
                O pegar una URL de imagen
              </summary>
              <input
                type="text"
                value={f.imagenes.startsWith('data:') ? '' : f.imagenes}
...
export default function AdminProductosPage() {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* CAMBIO 1.1: Header con botón "Nuevo producto" grande y accesible */}
      <div className="flex items-center justify-between flex-wrap gap-4">
        ...
        {/* CAMBIO 1.1: Botón prominente, separado, con sombra y padding generoso */}
        <button
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-3">
                        <div className="w-10 h-10 rounded-lg overflow-hidden flex-shrink-0 bg-melocoton-50 relative">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center text-lg text-melocoton-300 -z-10">
                          </div>
                        </div>
          ...
          <div className="px-4 py-2.5 border-t border-gray-100 bg-gray-50/50">
            <p className="text-xs text-tierra-400">
              Click en el badge de stock para editarlo directamente · Click en columnas para ordenar
            </p>
          </div>
        </div>
      )}
